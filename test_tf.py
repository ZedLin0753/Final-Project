...
X = tf.placeholder(tf.float32, [None, 13328])
def make_cnn():
...
def sess_ocr(im):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, tf.train.latest_checkpoint('D:/python_opencv/code/19/models'))#/cnn_tf.ckpt
...
def ocr_handle(filename):
    ...
    print(filename)
    X = tf.placeholder(tf.float32, [None,13328])#17*28*28
    print(X)
